...
// --- Dashboard View ---
function DashboardView({ teams, seasonSimulated, onSimSeason, onRandomize, onNav }) {
...
export default function HockeySimGame() {
  const [leagueState, setLeagueState] = useState(() => {
    ...
    return { teams, freeAgents, currentView: 'dashboard', seasonSimulated: false, simming: false };
  });

  const { teams, freeAgents, currentView, seasonSimulated, simming } = leagueState;
  ...
  function handleRandomize() {
    ...
    setLeagueState({ teams: newTeams, freeAgents: newFAs, currentView: 'dashboard', seasonSimulated: false, simming: false });
  }

  function handleSimSeason() {
    setLeagueState(prev => ({ ...prev, simming: true }));
    // Use setTimeout to allow React to re-render the loading state
    setTimeout(() => {
  ...
  function handleTradeComplete(teamAId, teamBId, tradeAIds, tradeBIds) {
    setLeagueState(prev => {
      const newTeams = prev.teams.map(t => {
        if (t.id === teamAId) {
          // Remove players going to B, add players from B
          const pToB = t.players.filter(p => tradeAIds.includes(p.id)).map(p => ({ ...p, teamId: teamBId }));
          const incoming = prev.teams.find(tb => tb.id === teamBId)?.players.filter(p => tradeBIds.includes(p.id)).map(p => ({ ...p, teamId: teamAId })) || [];
          const remaining = t.players.filter(p => !tradeAIds.includes(p.id));
          return { ...t, players: [...remaining, ...incoming] };
        }
        if (t.id === teamBId) {
          const incoming = prev.teams.find(ta => ta.id === teamAId)?.players.filter(p => tradeAIds.includes(p.id)).map(p => ({ ...p, teamId: teamBId })) || [];
          const remaining = t.players.filter(p => !tradeBIds.includes(p.id));
          return { ...t, players: [...remaining, ...incoming] };
        }
        return t;
      });
  ...
  function handleSign(teamId, faId) {
    setLeagueState(prev => {
      ...
      const newTeams = prev.teams.map(t => {
        ...
        if (t.players.length >= 23) return t;
        // Assign to appropriate extra slot
        const signedPlayer = { ...fa, teamId, isExtra: true, lineNumber: 4 };
        return { ...t, players: [...t.players, signedPlayer] };
      });
  ...
  function handleRelease(teamId, playerId) {
    setLeagueState(prev => {
      ...
      const releasedPlayer = { ...player, teamId: null, isExtra: false };
      const newTeams = prev.teams.map(t => {
        if (t.id !== teamId) return t;
        return { ...t, players: t.players.filter(p => p.id !== playerId) };
      });
      return { ...prev, teams: newTeams, freeAgents: [...prev.freeAgents, releasedPlayer] };
    });
  }

  const navItems = [
    { key: 'dashboard', label: 'Dashboard', icon: '🏠' },
    { key: 'roster', label: 'Teams & Rosters', icon: '👥' },
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white" style={{ fontFamily: "'Segoe UI', system-ui, sans-serif" }}>
      ...
      <header className="bg-gray-900 border-b border-gray-800 px-4 py-3 flex items-center justify-between">
        ...
        {seasonSimulated && <div className="text-green-400 text-xs font-semibold bg-green-900 px-2 py-1 rounded">Season Complete</div>}
      </header>
        ...
        <div className="flex gap-1 py-1">
          {navItems.map(({ key, label, icon }) => (
            <button key={key} onClick={() => setView(key)}
              className={`px-3 py-2 rounded text-xs font-semibold whitespace-nowrap transition ${currentView === key ? 'bg-blue-700 text-white' : 'text-gray-400 hover:bg-gray-800 hover:text-white'}`}>
              {icon} {label}
            </button>
          ))}
        </div>
      ...
      <main className="max-w-7xl mx-auto">
        ...
        {currentView === 'simSeason' && (
          <div className="p-8 text-center">
            ...
            <button onClick={handleSimSeason} disabled={simming}
            ...
            {seasonSimulated && (
              <div className="mt-6 text-green-400">
                ✓ Season simulated! View standings on Dashboard or stats in Season Stats.
              </div>
            )}
          </div>
        )}
        {currentView === 'stats' && <SeasonStatsView teams={teams} seasonSimulated={seasonSimulated} />}
        {currentView === 'trades' && <TradeView teams={teams} onTradeComplete={handleTradeComplete} />}
